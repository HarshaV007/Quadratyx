...
for suit in suits:
    for rank in ranks:
        Card_deck = ""+rank+""+ ' of ' +""+suit+""
        l.append(Card_deck)
        random.shuffle(l)
print(l)
Numbercards=[item for item in l if item not in Facecards]
res = list(map(lambda sub:int(''.join(
      [ele for ele in sub if ele.isnumeric()])), Numbercards))
d1 = dict(zip(Numbercards,res))
d2=dict(zip(Facecards,[10] * 16))
Points_dict={**d1, **d2}
print(Points_dict)
Player_cards = random.choices(l,k=2)
Player_points = Points_dict[Player_cards[0]]+Points_dict[Player_cards[1]]
Remaining_cards = [item for item in l if item not in Player_cards]
Dealer_cards = random.choices(Remaining_cards,k=2)
Dealer_points = Points_dict[Dealer_cards[0]]+Points_dict[Dealer_cards[1]]
